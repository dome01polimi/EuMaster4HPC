...
@rfm.simple_test
class BasisSetScalingTest(rfm.RunOnlyRegressionTest):

    calculation = parameter(["opt", "dmc", "vmc"])
    basis_set = parameter(["DZ", "TZ", "QZ", "5Z"])
    ...
    def setup_execution(self):
        ...
        self.env_vars = {
            ...
            'QMECHA_TEST_DIR': 'tests/PerformanceTests/BasisSetScalingW5/',
            'CURR_ENV' : self.current_environ.name
        }

        # Prepare input data 
        os.system(f"cd {self.stagedir}")
        os.system(f"cp -r {self.env_vars['QMECHA_TEST_DIR']}start/{self.basis_set}/* {self.stagedir}/")
        ...
        self.executable_opts = [
            f"{self.env_vars['QMECHA_EXE']} -i input -m *.xyz -b *.bas -p pseudo.dat -j ma -sn BasisSetScaling_{self.calculation}_{self.basis_set.lower()}"
        ]
    ...
    def info(self):
        return f'%calculation:{self.calculation}%basis_set:{self.basis_set}'
